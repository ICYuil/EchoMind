...
本系统采用"三阶段大模型协同"的方式完成智能面试任务，整体架构为：**Qwen2.5-Omni-7B → Qwen2.5-7B → Fun-CosyVoice3-0.5B**

- **Qwen2.5-Omni-7B (ASR模型)**: 负责语音转写与情感识别，通过微调提升特定领域字准率与自信度分析能力
- **Qwen2.5-7B (核心LLM)**: 负责面试问题生成、追问、答案分析与评分，通过 Qwen3.5-plus 蒸馏方式进行能力迁移
- **Fun-CosyVoice3-0.5B (TTS模型)**: 负责将系统回复转换为自然语音输出，采用零样本说话人适配实现个性化音色

**GitHub 说明**: 由于模型文件较大，微调后的模型权重未包含在 GitHub 仓库中。本地使用时，系统默认通过调用 API 实现相关功能。需配置 API 密钥（如阿里云百炼）后即可使用。

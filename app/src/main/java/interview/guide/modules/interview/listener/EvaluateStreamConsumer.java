...
public class EvaluateStreamConsumer extends AbstractStreamConsumer<EvaluateStreamConsumer.EvaluatePayload> {
    ...
    public EvaluateStreamConsumer(
            ...
            SessionToBaseIdReposity sessionToBaseIdReposity
    ) {
        super(redisService);
        ...
        this.sessionToBaseIdReposity=sessionToBaseIdReposity;
    }
    ...
    protected EvaluatePayload parsePayload(StreamMessageId messageId, Map<String, String> data) {
        ...
        if (userIdStr != null && !userIdStr.isBlank()) {
            try {
                ...
                log.warn("无法解析userId: sessionId={}, userIdStr={}", sessionId, userIdStr);
            }
        }
        return new EvaluatePayload(sessionId, userId);
    }
    ...
    protected void processBusiness(EvaluatePayload payload) {
        ...
        try {
            ...
            log.error("解析问题JSON失败: sessionId={}, error={}", sessionId, e.getMessage(), e);
            throw new RuntimeException("解析问题JSON失败: " + e.getMessage(), e);
        }
        ...
        List<SessionToBaseEntity>entities=sessionToBaseIdReposity.findSessionToBaseEntitiesBySessionId(sessionId);
        List<Long>knowledgeBaseIds=entities.stream()
                .map(SessionToBaseEntity::getKnowledgeBaseId)
                .collect(Collectors.toList());
        InterviewReportDTO report = evaluationService.evaluateInterview(sessionId, resumeText, questions, knowledgeBaseIds, userId);
    ...
    protected void retryMessage(EvaluatePayload payload, int retryCount) {
        ...
        try {
            ...
            message.put(AsyncTaskStreamConstants.FIELD_SESSION_ID, sessionId);
            message.put(AsyncTaskStreamConstants.FIELD_RETRY_COUNT, String.valueOf(retryCount));
            if (userId != null) {
                message.put(AsyncTaskStreamConstants.FIELD_USER_ID, String.valueOf(userId));
            }

...
public class AnswerEvaluationService {
    ...
    private final PromptTemplate summaryLearningPlanTemplate;
    ...
    private final PromptTemplate userTestPromptTemplate;
    private final BeanOutputConverter<TestQuestionDTO> TestoutputConverter;
    ...
    public AnswerEvaluationService(
            ...
            @Value("classpath:prompts/interview-evaluation-learning-plan-system.st") Resource learningPlanResource,
            @Value("classpath:prompts/test-question-user.st") Resource userTestPromptTemplate,
        ...
        this.summaryLearningPlanTemplate=new PromptTemplate(learningPlanResource.getContentAsString(StandardCharsets.UTF_8));
        ...
        this.userTestPromptTemplate = new PromptTemplate(userTestPromptTemplate.getContentAsString(StandardCharsets.UTF_8));
        this.TestoutputConverter = new BeanOutputConverter<>(TestQuestionDTO.class);
this.testQuestionRepository=testQuestionRepository;
    }


    private record TestQuestionDTO(
            List<TestQuestionEntity>questionDTOS
    ) {}
    private List<String> optimizeLearningPlanWithAI(List<String> improvements, List<Document> documents) {
        if (documents == null || documents.isEmpty()) {
            return List.of("暂无针对性建议，请继续加强基础练习。");
        }

        // 1. 准备参考资料字符串（限制长度，防止超出 Context Window）
        String rawDocs = documents.stream()
                .map(Document::getText)
                .collect(Collectors.joining("\n---\n"));

        if (rawDocs.length() > 3000) { // 简单截断或做预处理
            rawDocs = rawDocs.substring(0, 3000) + "...";
        }

        // 2. 渲染 Prompt
        Map<String, Object> variables = new HashMap<>();
        variables.put("improvements", String.join(", ", improvements));
        variables.put("rawDocuments", rawDocs);
        String userPrompt = summaryLearningPlanTemplate.render(variables);

        try {
            // 3. 调用 AI (这里如果不需要结构化 DTO，直接用 chatClient 获取 String)
            String aiResponse = chatClient.prompt()
                    .system("你是一位专业的编程导师，负责生成结构化的学习计划。")
                    .user(userPrompt)
                    .call()
                    .content();

            // 4. 将 AI 生成的 Markdown 文本按行拆分，或者直接作为一个整体
            // 如果 InterviewReportDTO 的 learningPlan 是 List<String>，建议按条目拆分
            return Arrays.stream(aiResponse.split("\n"))
                    .filter(line -> !line.isBlank())
                    .collect(Collectors.toList());

        } catch (Exception e) {
            log.warn("AI 优化学习计划失败，返回原始检索内容: {}", e.getMessage());
            return documents.stream().map(Document::getText).limit(5).toList();
        }
    }


private TestQuestionDTO getTestQs(List<String>list){
        Map<String,Object>variables=new HashMap<>();
        variables.put("learningPlan",list);
        String userPrompt=userTestPromptTemplate.render(variables);
    // 添加格式指令到系统提示词
    String systemPromptWithFormat = userPrompt + "\n\n" + TestoutputConverter.getFormat();

    TestQuestionDTO dto = null;
        try {
    ...
    public InterviewReportDTO evaluateInterview(String sessionId, String resumeText,
                                                List<InterviewQuestionDTO> questions,List<Long>knowledgeBaseIds) {
        return evaluateInterview(sessionId, resumeText, questions, knowledgeBaseIds, null);
    }

    /**
     * 评估完整面试并生成报告（支持传入userId，用于异步任务）
     */
    public InterviewReportDTO evaluateInterview(String sessionId, String resumeText,
                                                List<InterviewQuestionDTO> questions, List<Long> knowledgeBaseIds, Long userId) {
...
//            List<Document> learningPlanUnfinshed = knowledgeBaseQueryService.getLearningPlan(knowledgeBaseIds, finalSummary.improvements);
//            List<String> learningPlan=learningPlanUnfinshed.stream()
//                    .map(Document::getText)
//                    .collect(Collectors.toList());

            List<Document> learningPlanUnfinished = userId != null
                    ? knowledgeBaseQueryService.getLearningPlan(knowledgeBaseIds, finalSummary.improvements(), userId)
                    : knowledgeBaseQueryService.getLearningPlan(knowledgeBaseIds, finalSummary.improvements());

// 使用 AI 进行内容优化和排版
            List<String> learningPlan = optimizeLearningPlanWithAI(finalSummary.improvements(), learningPlanUnfinished);
            for(Long i:knowledgeBaseIds){
                if(!testQuestionRepository.existsByDatabaseId(i)){
                    TestQuestionDTO testQs=getTestQs(learningPlan);
                    List<TestQuestionEntity>list=testQs.questionDTOS();
                    for(TestQuestionEntity q:list){
                        q.setDatabaseId(knowledgeBaseIds.get(0));
                        q.setId(null);
                    }
                    // 重要：在返回前，统一设置 databaseId 并清除 AI 可能误生成的 ID
                    testQuestionRepository.saveAll(list);
                }
            }
    ...
    private InterviewReportDTO convertToReport(
        ...
        return new InterviewReportDTO(
                ...
                referenceAnswers,
                learningPlan !=null ?learningPlan:List.of()
        );
    }
}

...
public class RagChatController {
    ...
    public Flux<ServerSentEvent<String>> sendMessageStream(
        ...
        // 1. 准备消息（保存用户消息，创建 AI 消息占位，并获取知识库ID）
        var prepareResult = sessionService.prepareStreamMessage(sessionId, request.question());
        Long messageId = prepareResult.messageId();
        List<Long> kbIds = prepareResult.kbIds();

        // 2. 获取流式响应（使用预先获取的知识库ID，避免在流式响应中丢失请求上下文）
        StringBuilder fullContent = new StringBuilder();

        return sessionService.getStreamAnswer(kbIds, request.question())

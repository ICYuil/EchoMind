...
public class RagChatSessionService {
     ...
     *
     * @return 包含消息ID和知识库ID列表的结果
     */
    @Transactional
    public PrepareStreamMessageResult prepareStreamMessage(Long sessionId, String question) {
        Long userId = securityUtils.currentUserOrThrow().userId();
        ...
        return new PrepareStreamMessageResult(assistantMessage.getId(), kbIds);
    }
    ...
    /**
     * 获取流式回答
     * 注意：此方法不接受sessionId，而是直接接受知识库ID列表，
     * 避免在流式响应中丢失请求上下文导致401错误
     */
    public Flux<String> getStreamAnswer(List<Long> kbIds, String question) {
     ...
     * 准备流式消息的结果
     */
    public record PrepareStreamMessageResult(Long messageId, List<Long> kbIds) {}
}
